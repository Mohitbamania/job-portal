                    ...
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                        ...
                        <div class="card-body p-4">
                            <div class="table-responsive">
                                <table id="appliedJobTable" class="table table-modern align-middle mb-0">
                                    ...
                                    <tbody>
                                        @forelse($appliedJob as $job)
                                            <tr>
                                                <td>
                                                    <div class="fw-semibold text-dark">{{ $job->jobDetail->title }}</div>
                                                    <small class="text-muted">{{ $job->jobDetail->location }}</small>
                                                </td>
                                                <td>{{ \Carbon\Carbon::parse($job->created_at)->format('d M, Y') }}</td>
                                                <td>
                                                    <span class="badge bg-info-subtle text-info fw-semibold">
                                                        {{ $job->jobDetail->applications_count ?? 0 }} Applications
                                                    </span>
                                                </td>
                                                <td>
                                                    @if($job->jobDetail->status == 1)
                                                ...
                                                <td class="text-center">
                                                    <div class="action-buttons d-flex justify-content-center gap-1">
                                                        <!-- View Button -->
                                                        <a href="{{ route('account.jobOverview', $job->jobDetail->id) }}"
                                                            class="btn-action btn-view" data-bs-toggle="tooltip"
                                                            title="View Job">
...
@section('customJS')
    <script type="text/javascript">
        function deleteJob(jobId) {
            Swal.fire({
                title: 'Are you sure?',
                text: "This will remove the applied job from your list!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('job.appliedJobDelete') }}",
                        type: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
                            jobId: jobId
                        ...
                        success: function (response) {
                            if (response.success == true) {
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Removed!',
                                    text: 'The applied job has been removed successfully.',
                                ...
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Error',
                                    text: 'Something went wrong while removing the job.',
